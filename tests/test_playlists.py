...
def sample_playlist():
    return Playlist(
        ...
        songs=[
            PlaylistSong(
                song_id=1, title="Dream On", artist="Aerosmith", year=1973, spotify_id="spotify1"
            ),
            PlaylistSong(
                song_id=2, title="Dreams", artist="Fleetwood Mac", year=1977, spotify_id="spotify2"
            ),
            PlaylistSong(
                song_id=3, title="Sweet Dreams", artist="Eurythmics", year=1983, spotify_id=None
            ),
        ],
    )
...
def test_search_by_title_matches_title(tmp_path, monkeypatch):
    ...
    importlib.reload(
        __import__("music_teacher_ai.playlists.manager", fromlist=["_search_by_title"])
    )
