...
_DEMO_JSON = (
    Path(__file__).resolve().parent.parent / "music_teacher_ai" / "demo" / "demo_songs.json"
)
...
class TestDemoDataset:
    ...
    def test_lyrics_are_non_empty(self):
        data = json.loads(_DEMO_JSON.read_text(encoding="utf-8"))
        for entry in data:
            assert len(entry["lyrics"].strip()) > 10, f"Lyrics too short for {entry['title']}"
...
class TestLoadDemoSongs:
    ...
    def test_shared_artists_deduplicated(self, fresh_db):
        ...
        with get_session() as session:
            beatles = session.exec(select(Artist).where(Artist.name == "The Beatles")).all()
